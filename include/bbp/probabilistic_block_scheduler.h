...
class ProbabilisticBlockScheduler {
 public:
  ...
  class PreparedRemove {
   public:
    PreparedRemove(PreparedRemove&& other) noexcept;
    PreparedRemove& operator=(PreparedRemove&&) = delete;

    PreparedRemove(const PreparedRemove&) = delete;
    PreparedRemove& operator=(const PreparedRemove&) = delete;
   ...
   private:
    ...
    PreparedRemove(ProbabilisticBlockScheduler* owner,
                   std::unique_lock<std::mutex> lock,
                   std::vector<std::string> miner_node_ids,
                   std::vector<bool> active_miners,
                   std::vector<bool> in_flight_miners)
 ...
 private:
  ...
  bool membership_mutation_pending_ = false;
  bool started_ = false;
};
