...
boost::json::object SignalValueSchema() {
  auto name = StringSchema(1U);
  name["description"] =
      "Host signal name (SIGTERM, SIGSTOP, SIGCONT, SIGRTMIN+n, SIGRTMAX-n) or "
      "exact decimal string; shared host parser rejects unknown names and "
      "out-of-range values.";
  name["pattern"] = "^(SIG[A-Z0-9]+([+-][0-9]+)?|[0-9]+)$";
  return OneOf({IntegerSchema(1U, NSIG - 1U), std::move(name)});
}

boost::json::object SignalScopeSchema() {
  auto schema =
      StringEnumSchema(boost::json::array{"process", "process_group"});
  schema["description"] =
      "Explicit owned daemon or its private process group. Group delivery "
      "requires Linux 6.9; kernel errors return without fallback.";
  return schema;
}

boost::json::object SignalDeliverySchema() {
  boost::json::object observed{
      {"state",
       StringEnumSchema(boost::json::array{"running", "stopped", "exited"})},
      ...
      {"core_dumped", TypeSchema("boolean")}};
  observed.at("state").as_object()["description"] =
      "Observed leader state, including for group delivery. Running means "
      "alive "
      "without an observed job-control stop; it does not imply RPC readiness "
      "or that the delivered signal has taken effect.";
  boost::json::object properties{
      {"target", StringEnumSchema(
                     boost::json::array{"node_daemon", "wallet_node_daemon"})},
      {"wallet", ClosedObject({{"wallet_index", IntegerSchema(1U)},
                               {"node_id", NodeAddIdentifierSchema()},
                               {"registry_generation", Uint64Schema()}},
                              Required({"wallet_index", "node_id",
                                        "registry_generation"}))},
      {"target_pid", IntegerSchema(1U)},
      {"process_group_id", IntegerSchema(1U)},
      {"signal", IntegerSchema(1U, NSIG - 1U)},
      {"signal_name", StringSchema(1U)},
      {"scope", SignalScopeSchema()},
      {"kernel_result",
       boost::json::object{{"type", "integer"},
                           {"enum", boost::json::array{-1, 0}}}},
      {"errno", IntegerSchema()},
      {"accepted", TypeSchema("boolean")},
      {"observation",
       ClosedObject(std::move(observed),
                    Required({"state", "restart_count", "restart_policy",
                              "restart_action", "cleanup_state"}))},
      {"evidence_families", ArraySchema(StringSchema(1U), 1U)}};
  auto schema = ClosedObject(
      std::move(properties),
      Required({"target", "target_pid", "process_group_id", "signal",
                "signal_name", "scope", "kernel_result", "errno", "accepted",
                "observation", "evidence_families"}));
  schema["if"] = boost::json::object{
      {"properties", boost::json::object{
                         {"target", ConstStringSchema("wallet_node_daemon")}}}};
  schema["then"] = boost::json::object{{"required", Required({"wallet"})}};
  schema["description"] =
      "A wallet_node_daemon target shares its process with all backing node "
      "roles.";
  return schema;
}
...
boost::json::object RoleMutationRequestSchema(bool assigning,
                                              bool include_run_id) {
  boost::json::object properties;
  boost::json::array required;
  if (include_run_id) {
  ...
  properties["node_ids"] =
      ArraySchema(NodeAddIdentifierSchema(), 1U,
                  assigning ? kSimulationNodeAddMaximumCount
           ...
           boost::json::object{
               {"not",
                boost::json::object{
                    {"anyOf",
                     boost::json::array{
                         boost::json::object{{"required", Required({"mode"})}},
                         boost::json::object{
                             {"required",
...
boost::json::object CommandVariant(SimulationCommandKind kind,
  ...
  if (properties.contains("role_mutation")) {
    properties["role_mutation"] = RoleMutationRequestSchema(
        kind == SimulationCommandKind::kAssignRole, false);
  }
...
boost::json::object WalletMutationIdentitySchema() {
  boost::json::object properties;
  ...
  properties["funding_address"] = StringSchema(1U);
  return ClosedObject(std::move(properties),
                      Required({"wallet_index", "node", "node_id", "mode",
...
boost::json::object NodeRemoveConfigSchema() {
  boost::json::object properties;
  properties["node_ids"] = ArraySchema(NodeAddIdentifierSchema(), 1U,
                                       kSimulationNodeRemoveMaximumCount, true);
...
boost::json::object BuildMcpOperationInputSchema(
  ...
  switch (operation) {
    ...
    case McpOperationKind::kSignalWallet:
      ...
      add_node();
      add_timeout();
      properties["signal"] = SignalValueSchema();
      properties["scope"] = SignalScopeSchema();
      required.emplace_back("signal");
      required.emplace_back("scope");
      break;
    case McpOperationKind::kStopNode:
    case McpOperationKind::kKillNode:
    case McpOperationKind::kRestartNode:
    ...
    case McpOperationKind::kRemoveWallet:
      ...
      break;
    case McpOperationKind::kAssignRole:
      return AddDraft(RoleMutationRequestSchema(true, true));
    case McpOperationKind::kRemoveRole:
      return AddDraft(RoleMutationRequestSchema(false, true));
    case McpOperationKind::kAddMiner:
      add_run();
      properties["node_ids"] = ArraySchema(
  ...
  boost::json::object schema =
      ClosedObject(std::move(properties), std::move(required));
  if (operation == McpOperationKind::kRemoveWallet) {
    schema["oneOf"] = boost::json::array{
        boost::json::object{
            {"required", Required({"node_id"})},
            {"not", boost::json::object{{"required", Required({"node_ids"})}}}},
        boost::json::object{
            {"required", Required({"node_ids"})},
            {"not", boost::json::object{{"required", Required({"node_id"})}}}}};
  }
...
boost::json::object SignalResultSchema(McpOperationKind operation) {
  auto schema = BuildMcpResultSchema(McpResultFamily::kRuntimeCommand);
  schema.at("properties").as_object()["action"] =
      ConstStringSchema(McpOperationKindName(operation));
  auto& required = schema.at("required").as_array();
  required.emplace_back("action");
  required.emplace_back("signal_delivery");
  required.emplace_back("affected_node_ids");
  auto& delivery =
      schema.at("properties").as_object().at("signal_delivery").as_object();
  ...
  if (operation == McpOperationKind::kSignalWallet) {
    ...
    delivery["description"] =
        "Signals the wallet backing node daemon and affects all its roles. "
        "Observed state describes the daemon leader, not every group member.";
  }
  return schema;
}
...
boost::json::object NodeRemoveMutationSchema() {
  boost::json::object schema = BuildMcpResultSchema(McpResultFamily::kMutation);
  boost::json::object& properties = schema.at("properties").as_object();
  properties["added_node_ids"] =
      ArraySchema(NodeAddIdentifierSchema(), 0U, 0U, true);
  properties["removed_node_ids"] = ArraySchema(
      NodeAddIdentifierSchema(), 1U, kSimulationNodeRemoveMaximumCount, true);
  properties["affected_node_ids"] = ArraySchema(
      NodeAddIdentifierSchema(), 1U, kSimulationNodeRemoveMaximumCount, true);
  properties["action"] = ConstStringSchema("node.remove");
  properties["inventory_generation"] = IntegerSchema();
...
boost::json::object RoleRemoveMutationSchema() {
  boost::json::object schema =
      BuildMcpResultSchema(McpResultFamily::kRoleMutation);
  boost::json::object& properties = schema.at("properties").as_object();
  properties["node_ids"] = ArraySchema(NodeAddIdentifierSchema(), 1U,
                                       kSimulationNodeRemoveMaximumCount, true);
  properties["assigned_roles"] = ArraySchema(RoleSchema(), 0U, 0U, true);
  properties["removed_roles"] = ArraySchema(
      StringEnumSchema(boost::json::array{"wallet", "miner", "masternode"}), 1U,
      1U, true);
  properties["action"] = ConstStringSchema("role.remove");
  properties["state"] = ConstStringSchema("removed");
  properties["created_node_ids"] =
      ArraySchema(NodeAddIdentifierSchema(), 0U, 0U, true);
  return schema;
}

boost::json::object EmbeddedRoleMutationResultSchema(
    boost::json::object schema) {
  schema.erase("$schema");
  boost::json::object& properties = schema.at("properties").as_object();
  boost::json::array& required = schema.at("required").as_array();
  for (const std::string_view field :
       {"result_family", "run_id", "operation_id"}) {
...
boost::json::object RuntimeRoleMutationResultSchema() {
  boost::json::array variants;
  variants.emplace_back(
      EmbeddedRoleMutationResultSchema(RoleAssignMutationSchema()));
  variants.emplace_back(
      EmbeddedRoleMutationResultSchema(RoleRemoveMutationSchema()));
  return boost::json::object{{"oneOf", std::move(variants)}};
}

boost::json::object WalletRemoveMutationSchema() {
  boost::json::object schema = BuildMcpResultSchema(McpResultFamily::kMutation);
  boost::json::object& properties = schema.at("properties").as_object();
  properties["added_node_ids"] =
      ArraySchema(NodeAddIdentifierSchema(), 0U, 0U, true);
  ...
  properties["affected_node_ids"] = ArraySchema(
      NodeAddIdentifierSchema(), 1U, kSimulationNodeRemoveMaximumCount, true);
  properties["action"] = ConstStringSchema("wallet.remove");
  properties["state"] = ConstStringSchema("removed");
  properties["unchanged"] =
      boost::json::object{{"type", "boolean"}, {"const", false}};
  properties["wallets"] =
      ArraySchema(WalletMutationIdentitySchema(), 1U, kMaximumSafeCollection);
  properties["wallet_generation"] = Uint64Schema(1U);
...
boost::json::object BuildMcpResultSchema(
  ...
  switch (family) {
    ...
    case McpResultFamily::kRuntimeCommand:
      properties["signal_delivery"] = SignalDeliverySchema();
      constraints.emplace_back(boost::json::object{
          ...
          {"then", boost::json::object{
                       {"required",
                        Required({"signal_delivery", "affected_node_ids"})}}}});
      properties["run_id"] = RunIdentifierSchema();
      properties["command_id"] = IdentifierSchema();
      properties["operation_id"] = IdentifierSchema();
      ...
      constraints.emplace_back(boost::json::object{
          {"if", boost::json::object{{"properties",
                                      boost::json::object{
                                          {"action",
                                           ConstStringSchema("node.remove")}}},
                                     {"required", Required({"action"})}}},
          {"then",
           boost::json::object{
               {"required",
                Required({"removed_node_ids", "affected_node_ids",
                          "inventory_generation", "final_node_count"})}}}});
      for (const std::string_view action : {"role.assign", "role.remove"}) {
        constraints.emplace_back(boost::json::object{
            {"if",
             boost::json::object{
                 {"properties",
                          ...
                          boost::json::object{
                              {"role_mutation",
                               boost::json::object{
                                   {"properties",
                                    boost::json::object{
                                        {"action", ConstStringSchema(action)}}},
                                   {"required", Required({"action"})}}}}},
                         {"required", Required({"role_mutation"})}}}});
      }
      constraints.emplace_back(boost::json::object{
          {"if",
           boost::json::object{{"required", Required({"role_mutation"})}}},
          {"then", boost::json::object{
                       {"properties",
                        boost::json::object{
                            {"action", StringEnumSchema(boost::json::array{
    ...
    case McpResultFamily::kRoleMutation:
      ...
      }
      {
        constraints.emplace_back(boost::json::object{
            {"if",
             boost::json::object{
                 {"properties",
                  boost::json::object{
                      {"action", ConstStringSchema("role.remove")}}},
                 {"required", Required({"action"})}}},
            {"then",
             boost::json::object{
                 {"properties",
                  boost::json::object{
                      ...
                      {"removed_roles",
                       ArraySchema(StringEnumSchema(boost::json::array{
                                       "wallet", "miner", "masternode"}),
                                   1U, 1U, true)},
                      {"state", ConstStringSchema("removed")},
                      {"created_node_ids",
                       ArraySchema(NodeAddIdentifierSchema(), 0U, 0U, true)}}},
                 {"required",
                  Required({"state", "created_node_ids", "role_generation",
        ...
        const auto add_role_remove_constraint =
            [&](std::string_view role,
                std::initializer_list<std::string_view> required_fields,
                std::initializer_list<std::string_view> forbidden) {
              boost::json::object then_schema{
              ...
              boost::json::object role_properties;
              if (role == "wallet") {
                role_properties["final_wallet_count"] = IntegerSchema();
                role_properties["final_wallet_node_count"] = IntegerSchema();
                role_properties["wallets"] = ArraySchema(
                    WalletMutationIdentitySchema(), 1U, kMaximumSafeCollection);
              } else if (role == "miner") {
                role_properties["final_miner_count"] = IntegerSchema();
              } else {
                role_properties["final_masternode_count"] = IntegerSchema();
                role_properties["masternodes"] =
                    ArraySchema(MasternodeMutationIdentitySchema(), 1U,
                                kSimulationNodeRemoveMaximumCount);
              }
                        ...
                        boost::json::object{
                            ...
                            {"removed_roles",
                             boost::json::object{
                                 {"const",
                                  boost::json::array{std::string(role)}}}}}},
                       {"required", Required({"action", "removed_roles"})}}},
                  {"then", std::move(then_schema)}});
            };
        add_role_remove_constraint(
            "wallet",
            {"final_wallet_count", "final_wallet_node_count", "wallets"},
            {"final_miner_count", "final_masternode_count", "masternodes"});
        add_role_remove_constraint(
                 ...
                 boost::json::object{
                     {"properties", boost::json::object{
                                        {"terminal_result_family",
                                         ConstStringSchema("runtime_command")},
                                        {"terminal_result",
          ...
          constraints.emplace_back(boost::json::object{
              {"if",
               boost::json::object{
                   {"properties",
                    boost::json::object{
                        {"operation", ConstStringSchema("role.remove")},
                        {"state", ConstStringSchema("succeeded")}}},
                   {"required", Required({"operation", "state"})}}},
              {"then", boost::json::object{
                           {"properties",
          ...
          constraints.emplace_back(boost::json::object{
              {"if",
               boost::json::object{
                   {"properties",
                    boost::json::object{
                        {"operation", ConstStringSchema("wallet.remove")},
                        {"state", ConstStringSchema("succeeded")}}},
                   {"required", Required({"operation", "state"})}}},
              {"then",
               boost::json::object{
                   {"properties",
                    boost::json::object{
                        {"terminal_result_family",
                         ConstStringSchema("mutation")},
                        {"terminal_result", WalletRemoveMutationSchema()}}}}}});
          constraints.emplace_back(boost::json::object{
              {"if",
               boost::json::object{
                   {"properties",
