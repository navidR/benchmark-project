...
using simulator_app_internal::LiveWalletWorkloadJson;
...
using simulator_app_internal::TopologyEdgeRollbackFailureDetail;
using simulator_app_internal::TopologyEdgeUpdateDetail;
...
bool StartPreparedNode(const Options& options,
  ...
  return StartNodeProcessWithPolicy(options, events_path, driver, node, reason,
  ...
  for (std::size_t index = 0U; index < nodes.size(); ++index) {
    ...
    try {
      ...
      CleanupRuntimeNodeRpcCredential(
          RequireRunOwnership(options),
          RuntimeNodeResourceEntryFor(options, node.config, resource_slot,
                                      RuntimeNodeResourceState::kLive));
      if (node.process.running()) {
        throw std::runtime_error(
...
RuntimeNodeAddResult AddRuntimeNodesTransactional(
  ...
  for (std::uint32_t index = static_cast<std::uint32_t>(before.size());
    ...
    startup_configs[index].connect_peers = DynamicPhysicalTopologyPeerEndpoints(
  ...
  try {
    ...
    for (std::size_t index = 0U; index < node_ids.size(); ++index) {
      ...
      if (options.isolate_network) {
        ...
        directional_policies = DynamicDirectionalNetworkPolicies(
      ...
      pending_manifest.nodes.push_back(RuntimeNodeResourceEntryFor(
    ...
    for (std::size_t index = 0U; index < insertions.size(); ++index) {
      ...
      if (!StartPreparedNode(dynamic_options, staged_events_path, driver,
    ...
    const auto peer_visible = [&](std::uint32_t source_index,
      ...
      const std::string endpoint = NodePeerEndpoint(runtime_at(target_index));
      return !driver
    ...
    for (std::uint32_t first = 0U; first < final_count; ++first) {
      for (std::uint32_t second = first + 1U; second < final_count; ++second) {
        ...
        const bool should_connect =
            DynamicPhysicalPeerRequired(*next_runtime_topology, first, second);
    ...
    if (published) {
      ...
      try {
        ...
        throw SimulationCommandOutcomeUnconfirmed(
            ExceptionMessage(failure) + "; staged event cleanup failed: " +
            ExceptionMessage(std::current_exception()));
      }
...
RuntimeNodeReplaceResult ReplaceRuntimeNodeTransactional(
  ...
  {
    std::lock_guard<std::mutex> resource_lock(node_resource_state_mutex);
  ...
  const auto rollback = [&] {
    ...
    rollback_step("thaw original cgroup", [&] {
      if (target_frozen || target.cgroup->Frozen()) {
        target.cgroup->Thaw();
        if (!WaitForNodeFrozenState(*target.cgroup, false,
                                    rollback_stop_token)) {
          throw std::runtime_error(
    ...
    rollback_step("stop replacement candidate", [&] {
      ...
      try {
        StopNodeProcess(options, staged_events_path, driver, *candidate,
    ...
    if (root_restoration_verified && prior_process_restored &&
      ...
      rollback_step("restore resource manifest", [&] {
        ...
        const RuntimeNodeResourceManifest live =
            RuntimeNodeResourceManifestFor(options, inventory.Snapshot());
        const std::optional<RuntimeNodeResourceManifest> persisted =
            TryLoadRuntimeNodeResourceManifest(RequireRunOwnership(options));
  ...
  try {
    ...
    if (!WaitForNodeFrozenState(*target.cgroup, true, stop_token)) {
      throw std::runtime_error(
    ...
    target.cgroup->Thaw();
    if (!WaitForNodeFrozenState(*target.cgroup, false, stop_token)) {
      throw std::runtime_error(
    ...
    if (!StartNodeProcessWithPolicy(replacement_options, staged_events_path,
                                    driver, *candidate,
    ...
    StopNodeProcess(replacement_options, staged_events_path, driver, *candidate,
                    stop_token, true);
    ...
    StopNodeProcess(replacement_options, staged_events_path, driver, target,
                    stop_token);
    ...
    RuntimeNodeInventory::PreparedReplacement prepared_inventory =
        inventory.PrepareReplacement(
            ...
            final_configs);
    std::unique_lock<std::mutex> resource_lock(node_resource_state_mutex);
    std::unique_lock<std::mutex> network_lock(node_network_state_mutex);
    if (request.resources) {
...
BenchmarkHeadlessResult RunBenchmarkHeadless(
  ...
  try {
    ...
    const auto wait_for_peers_operation = [&,
      ...
      const auto expire_epoch_if_due =
          [&](LiveWaitForPeersWorkloadRecord& record) {
            if (record.completion_pending ||
                IsTerminalLiveWorkloadState(record.state) ||
                (record.state != LiveWorkloadState::kRunning &&
                 !(record.state == LiveWorkloadState::kStopping &&
      ...
      if (kind == McpOperationKind::kStartWorkload) {
        ...
        std::shared_ptr<LiveWaitForPeersWorkloadRecord> record;
        {
          auto mutation_lock = AcquireNodeMutationLock(node_mutation_mutex,
                                                       operation_stop_token);
          const RuntimeNodeSnapshot current_nodes = node_inventory.Snapshot();
          const WaitForPeersWorkload workload =
              ParseAndValidateLiveWaitForPeersWorkload(
                  workload_value->as_object(), options, current_nodes);
          record =
              launch_wait_for_peers_workload(workload, std::move(requested_id));
        }
        std::unique_lock<std::mutex> lock(record->mutex);
        if (!record->changed.wait(lock, operation_stop_token, [&] {
              return record->state != LiveWorkloadState::kStarting;
      ...
      const std::shared_ptr<LiveWaitForPeersWorkloadRecord> record =
          find_wait_for_peers_workload_record(workload_id);
      if (!record) {
        throw McpOperationFailure(
            "workload_not_found",
            "wait-for-peers workload instance is not registered: " +
                workload_id,
            false);
      }
      if (kind == McpOperationKind::kInspectWorkload) {
      ...
      if (kind == McpOperationKind::kReconfigureWorkload) {
        ...
        WaitForPeersWorkload updated;
        {
          auto mutation_lock = AcquireNodeMutationLock(node_mutation_mutex,
                                                       operation_stop_token);
          updated = ParseAndValidateLiveWaitForPeersWorkload(
              workload_value->as_object(), options, node_inventory.Snapshot());
        }
        bool publish_paused_revision = false;
        std::uint64_t expected_revision = 0U;
        {
          ...
          if (IsTerminalLiveWorkloadState(record->state)) {
            throw McpOperationFailure(
                "workload_not_active",
                "terminal wait-for-peers workload cannot be "
                "reconfigured",
                false);
          }
          if (expire_epoch_if_due(*record)) {
            throw McpOperationFailure(
                "workload_transition_in_progress",
                "wait-for-peers workload epoch deadline has elapsed", true);
          }
          if (record->state == LiveWorkloadState::kStarting ||
              record->state == LiveWorkloadState::kStopping) {
            throw McpOperationFailure(
                "workload_transition_in_progress",
                "wait-for-peers workload already has a lifecycle "
                "transition in progress",
                true);
          }
          if (record->configuration_revision ==
              std::numeric_limits<std::uint64_t>::max()) {
            throw std::runtime_error(
                "wait-for-peers workload configuration revision exceeds "
                "uint64");
          }
          expected_revision = record->configuration_revision;
          if (record->state == LiveWorkloadState::kPaused) {
      ...
      if (kind == McpOperationKind::kResumeWorkload) {
            ...
            std::chrono::steady_clock::now() + operation_timeout();
        {
          ...
          if (record->state != LiveWorkloadState::kPaused) {
            throw McpOperationFailure(
                "workload_not_paused",
                "wait-for-peers workload is not paused: " + workload_id, false);
          }
          record->request = LiveWorkloadRequest::kNone;
          record->state = LiveWorkloadState::kStarting;
          record->changed.notify_all();
        ...
        if (!record->changed.wait_until(
          ...
          throw McpOperationFailure(
              "workload_operation_timeout",
              "wait-for-peers workload did not resume before timeout", true);
        }
        lock.unlock();
        return LiveWaitForPeersWorkloadJson(*record);
      }
      ...
      bool already_paused = false;
      {
        ...
        if (IsTerminalLiveWorkloadState(record->state)) {
          throw McpOperationFailure(
              "workload_not_active",
              "wait-for-peers workload is already terminal: " + workload_id,
              false);
        }
        if (expire_epoch_if_due(*record)) {
          throw McpOperationFailure(
        ...
        if (kind == McpOperationKind::kPauseWorkload &&
          ...
          throw std::logic_error("unknown wait-for-peers workload operation");
        }
        record->changed.notify_all();
      }
      if (already_paused) {
      ...
      if (!record->changed.wait_until(lock, operation_stop_token, deadline,
        ...
        throw McpOperationFailure(
            "workload_operation_timeout",
            "wait-for-peers workload did not reach the requested state "
            "before timeout",
            true);
      }
      lock.unlock();
    ...
    workload_service->operation = [&, require_workload_record,
      ...
      const std::string workload_id = require_argument_string("workload_id");
      const std::shared_ptr<LiveWalletWorkloadRecord> record =
          require_workload_record(workload_id);
      if (kind == McpOperationKind::kInspectWorkload) {
        return LiveWalletWorkloadJson(*record);
      }
      if (kind == McpOperationKind::kReconfigureWorkload) {
        ...
        std::vector<std::uint32_t> updated_claims;
        {
          ...
          for (const auto& [other_id, other] : wallet_workloads->records) {
            ...
            if (!IsTerminalLiveWalletWorkloadState(other->state) &&
              ...
              throw McpOperationFailure(
                  "wallet_admission_conflict",
                  "reconfigured sender admission overlaps active "
                  "workload " +
                      other_id,
                  true);
            }
          ...
          if (updated.transaction_count != 0U &&
            ...
            throw McpOperationFailure(
                "workload_limit_conflict",
                "reconfigured maximum transaction count is below "
                "lifetime progress",
                false);
          }
          if (updated.strategy == WalletTransferStrategy::kEqualFanout &&
            ...
            throw McpOperationFailure(
                "workload_limit_conflict",
                "reconfigured equal_fanout maximum does not leave a "
                "complete receiver batch",
                false);
          }
      ...
      if (!record->changed.wait_until(lock, operation_stop_token, deadline,
        ...
        throw McpOperationFailure(
            "workload_operation_timeout",
            "wallet workload did not reach the requested state before "
            "timeout",
            true);
      }
      lock.unlock();
