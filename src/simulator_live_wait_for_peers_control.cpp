  ...
  return [find_record = std::move(find_record),
    ...
    if (!record) {
      throw McpOperationFailure(
          "workload_not_found",
          "wait-for-peers workload instance is not registered: " + workload_id,
          false);
    }
    ...
    if (kind == McpOperationKind::kReconfigureWorkload) {
      ...
      WaitForPeersWorkload updated = parse_reconfiguration(
      ...
      std::uint64_t expected_revision = 0U;
      {
        ...
        if (IsTerminalLiveWorkloadState(record->state)) {
          throw McpOperationFailure(
              "workload_not_active",
              "terminal wait-for-peers workload cannot be reconfigured", false);
        }
        ...
        if (record->state == LiveWorkloadState::kStarting ||
          ...
          throw McpOperationFailure(
              "workload_transition_in_progress",
              "wait-for-peers workload already has a lifecycle transition in "
              "progress",
              true);
        }
    ...
    if (!record->changed.wait_until(lock, operation_stop_token, deadline,
      ...
      throw McpOperationFailure(
          "workload_operation_timeout",
          "wait-for-peers workload did not reach the requested state before "
          "timeout",
          true);
    }
    lock.unlock();
