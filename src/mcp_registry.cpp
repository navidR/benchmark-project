...
constexpr auto kOperations = std::to_array<McpNamedCapability>(
    {{"scenario.validate", "Validate a complete typed scenario"},
     ...
     {"helper.signal",
      "Signal the selected node network-namespace keeper; namespace survives "
      "helper exit until cleanup. No helper restart or separate output "
      "capture"},
     ...
     {"role.assign",
      "Transactionally assign one compatible overlapping node role"},
     {"role.remove",
      "Transactionally remove one compatible overlapping node role"},
     {"miner.add", "Transactionally add or activate a miner"},
     {"miner.remove", "Deactivate and remove a miner role"},
     {"masternode.add", "Fund, register, configure and activate a masternode"},
     {"masternode.remove", "Deactivate and remove a masternode role"},
     {"masternode.restart", "Restart and verify a masternode role"},
     {"workload.invoke", "Invoke a registered one-shot workload action"},
...
constexpr auto kInformationFamilies = std::to_array<McpNamedCapability>(
...
constexpr auto kResultFamilies = std::to_array<McpNamedCapability>(
