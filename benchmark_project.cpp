...
constexpr const char MAX_IN_PEERS[] = "--in-peers={}";
constexpr const char MAX_OUT_PEERS[] = "--out-peers={}";
...
struct daemon {
    ...
    size_t max_log_files = 1;
    size_t max_concurrency = 4;
    size_t max_connections_per_ip = 18; // (2048 << 10); // 2 MB
    size_t max_in_peers = 7;
    size_t max_out_peers = 7;
...
std::string generate_block_header_by_height(const std::size_t _height) {
    ...
    boost::json::key_value_pair jsonrpc("jsonrpc", "2.0");
    boost::json::key_value_pair method("method", "get_block_header_by_height");
    boost::json::key_value_pair id("id", "2.0");
    boost::json::key_value_pair params("params", boost::json::object{{"height", _height}});

    boost::json::object rpc_request;
    rpc_request.insert(jsonrpc);
    rpc_request.insert(id);
    rpc_request.insert(method);
    rpc_request.insert(params);
    std::string request_json = boost::json::serialize(rpc_request);
    LTRACE << "Sending JSON-RPC request : " << boost::json::serialize(rpc_request);
    return request_json;
}
...
void run_wallets(std::atomic<bool>& wallet_stop_condition, std::size_t number_of_nodes, std::size_t test_duration)
{
    ...
    // Launch this command 
    // $ /usr/bin/gnome-terminal -- nload lo -t 200
    // with boost process.
        {
        try {
            filesystem::path term = process::search_path("gnome-terminal");
            if (term.empty()) {
                LWARNING << "gnome-terminal not found. Skipping nload monitor.";
            } else {
                // Redirect output to /dev/null (or keep to see nload's own output in the terminal)
                process::child net_mon(
                    term,
                    "--",          // everything after -- is the command executed by the terminal
                    "nload", "lo", "-t", "200",
                    process::std_out > process::null,
                    process::std_err > process::null
                );
                net_mon.detach(); // let it run independently
                LINFO << "Started network monitor: gnome-terminal -- nload lo -t 200";
            }
        } catch (const std::exception& e) {
            LERROR << "Failed to start nload monitor: " << e.what();
        }
    }
    ...
    auto wallet_transfer = [&](std::atomic<bool>& wallet_transfer_stop_condition, size_t index) {
        ...
        while (!wallet_transfer_stop_condition.load()) {
            ...
            // if (true) { // 0% chance to send money
            if (!(rand() % 3)) { // 66% chance to send money
...
void run_daemon(Daemon::daemon& daemon, size_t index, size_t seconds)
{
    ...
    process::child daemon_process(daemon.exec_path,
        ...
        std::format(Daemon::RPC_BIND_PORT_CONTAINER, std::to_string(daemon.rpc_port)),
        std::format(Daemon::MAX_CONCURRENCY_CONTAINER, std::to_string(daemon.max_concurrency)),
        std::format(Daemon::MAX_IN_PEERS, std::to_string(daemon.max_in_peers)),
        std::format(Daemon::MAX_OUT_PEERS, std::to_string(daemon.max_out_peers)),
        std::format(Daemon::DATA_DIR_CONTAINER, daemon.data_dir),
...
void run_daemons(std::atomic<bool>& daemon_stop_condition, std::size_t number_of_nodes, std::size_t test_duration)
{
    ...
    auto mimick_mining = [&](std::atomic<bool>& daemon_stop_condition, size_t index) {
        ...
        while (!daemon_stop_condition.load()) {
            ...
            if (rand() % 15) { // 6.66% chance to get block template
                std::this_thread::sleep_for(std::chrono::seconds(((rand() % UPDATE_INTERVAL))));
                continue;
            }
...
void status_reporter(std::atomic<bool>& stop_condition, std::size_t number_of_nodes)
{
    ...
    while (!stop_condition) {
        ...
        for (std::size_t index = 0; index < number_of_nodes; ++index) {
            ...
            std::size_t last10_sum_txes = 0;
            std::size_t last10_blocks_count = 0;
            std::array<std::size_t, 10> last10_each{};
            if (height > 0) {
                for (int offset = 1; offset <= 10 && height >= (std::size_t)offset; ++offset) {
                    try {
                        std::string request_json = generate_block_header_by_height(height - offset);
                        cpr::Url rpc_url { std::format("http://{}:{}/json_rpc", Daemon::RPC_DEFAULT_IP, Daemon::RPC_BASE_PORT + index) };
                        cpr::Response block_resp = cpr::Post(rpc_url, header, cpr::Body { request_json }, cpr::Timeout(TIMEOUTMS));
                        if (block_resp.status_code == 200) {
                            auto parsed_block = boost::json::parse(block_resp.text);
                            if (!parsed_block.as_object().contains("error")) {
                                std::size_t n = value_to<std::size_t>(
                                    parsed_block.at("result").at("block_header").at("num_txes"));
                                last10_each[offset - 1] = n;
                                last10_sum_txes += n;
                                last10_blocks_count++;
                            }
                        }
                    } catch (const std::exception& e) {
                        LTRACE << "Failed block header fetch h=" << (height - offset) << " N" << index << " : " << e.what();
                    }
                }
            }
            double last10_avg_txes = last10_blocks_count ? static_cast<double>(last10_sum_txes) / last10_blocks_count : 0.0;
            ...
            LINFO << "N" << index
                  ...
                  << ", last txis sum: " << last10_sum_txes
                  << ", last avg # txis : " << std::format("{:.2f}", last10_avg_txes)
